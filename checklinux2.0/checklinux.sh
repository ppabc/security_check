...
if [ $? -eq 0 ];then
  ...
  echo "没有/etc/lilo.conf文件" >> $savefile
fi
...
if [ $file6 = "-rw-------" ];then
  ...
  echo "没有/etc/xinetd.conf文件" >> $savefile
fi
...
if [ $? -eq 0 ];then
  ...
  do
    if [ $i = "soft" ];then
      echo "* soft core 0 已经设置" >> $savefile
    fi
    if [ $i = "hard" ];then
      echo "* hard core 0 已经设置" >> $savefile
    fi
  done
else 
...
if [ $? -eq 0 ];then
  ...
  echo "不已经设置远程root不能登陆，不符合要求，建议/etc/ssh/sshd_config添加PermitRootLogin no" >> $savefile
fi
...
if [ $telnetd = "yes" ];then
  ...
  echo "没有/etc/xinetd.d/telnet文件" >> $savefile
fi
...
do
  ...
  if [ $apend = "i" ];then
    echo "/etc/passwd文件存在i安全属性" >> $savefile
    flag=1
  fi
  if [ $apend = "a" ];then
...
do
  ...
  if [ $apend = "i" ];then
    echo "/etc/shadow文件存在i安全属性" >> $savefile
    flag=1
  fi
  if [ $apend = "a" ];then
...
do
  ...
  if [ $apend = "i" ];then
    echo "/etc/gshadow文件存在i安全属性" >> $savefile
    flag=1
  fi
  if [ $apend = "a" ];then
...
do
  ...
  if [ $apend = "i" ];then
    echo "/etc/group文件存在i安全属性" >> $savefile
    flag=1
  fi
  if [ $apend = "a" ];then
